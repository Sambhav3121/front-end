...
export default function Home() {
  ...
  const addToCartAPI = async (dishId) => {
    try {
      const response = await fetch('https://food-delivery.int.kreosoft.space/api/cart/add', {
        method: 'POST',
        headers: {
      ...
      // You can handle further actions after adding to the cart here
    } catch (error) {
          ...
          <div key={dish.id} className="col-md-3 col-sm-4 mb-4">
            <div className="card">
              ...
              <h5 className="card-title">{dish.name}</h5>
                ...
                <div className="rating">{renderStars(dish.rating)}</div>
              </div>
      ...
      {totalPages > 1 && (
        <div>
          {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
            <button key={page} disabled={page === currentPage} onClick={() => setCurrentPage(page)}>
              {page}
            </button>
          ))}
        </div>
